"""Tests for circular flow alert rule CLI."""
...
SCRIPT = ROOT / "scripts" / "run_circular_flow_rule.py"


def test_circular_flow_rule_cli_exists() -> None:
    assert SCRIPT.is_file()


def test_circular_flow_rule_cli_help_exits_zero() -> None:
    result = run(
...
def test_cli_run_help_includes_expected_options() -> None:
    ...
    assert "--base-risk-score" in result.stdout
    assert "--high-amount-risk-score" in result.stdout
