"""Tests for staged fan-in rule readers and runner."""
...
def _inputs(*args: object, **kwargs: object) -> tuple[pd.DataFrame, pd.DataFrame]:
    return build_fan_in_trigger_transactions_fixture(), build_fan_in_accounts_fixture()
...
def test_read_fan_in_rule_inputs_returns_transactions_and_accounts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "graph_aml.rules.staged.read_staged_transactions_for_rules",
        lambda *args, **kwargs: build_fan_in_trigger_transactions_fixture(),
    )
    monkeypatch.setattr(
        "graph_aml.rules.staged.read_staged_accounts_for_rules",
        lambda *args, **kwargs: build_fan_in_accounts_fixture(),
    )

    transactions, accounts = read_fan_in_rule_inputs(FakeEngine())

    assert len(transactions) == 15
    assert len(accounts) > 0


def test_run_fan_in_rule_from_staged_runs_rule_using_staged_inputs(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("graph_aml.rules.staged.read_fan_in_rule_inputs", _inputs)
    monkeypatch.setattr("graph_aml.rules.staged.write_rule_execution_audit_event", _no_audit)

    summary = run_fan_in_rule_from_staged(FakeEngine())

    assert summary["alerts_generated"] == 1


def test_run_fan_in_rule_from_staged_persists_only_when_requested(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[int] = []
    monkeypatch.setattr("graph_aml.rules.staged.read_fan_in_rule_inputs", _inputs)
    ...
    run_fan_in_rule_from_staged(FakeEngine(), persist=False)
    run_fan_in_rule_from_staged(FakeEngine(), persist=True)

    assert calls == [1]


def test_run_fan_in_rule_from_staged_writes_audit_only_when_requested(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    run_fan_in_rule_from_staged(FakeEngine(), write_audit=False)
    run_fan_in_rule_from_staged(FakeEngine(), write_audit=True)

    assert len(calls) == 1


def test_run_fan_in_rule_from_staged_uses_fan_in_audit_action(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[dict[str, object]] = []
    monkeypatch.setattr("graph_aml.rules.staged.read_fan_in_rule_inputs", _inputs)
    ...
    assert calls[0]["action"] == "run_fan_in_rule"
...
def test_run_fan_in_rule_from_staged_accepts_custom_config(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("graph_aml.rules.staged.read_fan_in_rule_inputs", _inputs)
    monkeypatch.setattr("graph_aml.rules.staged.write_rule_execution_audit_event", _no_audit)

    summary = run_fan_in_rule_from_staged(
        FakeEngine(),
