"""Tests for rapid movement rule CLI."""
...
SCRIPT = ROOT / "scripts" / "run_rapid_movement_rule.py"


def test_rapid_movement_rule_cli_exists() -> None:
    assert SCRIPT.is_file()


def test_rapid_movement_rule_cli_help_exits_zero() -> None:
    result = run(
...
def test_cli_run_help_includes_expected_options() -> None:
    ...
    assert "--outflow-window-hours" in result.stdout
    assert "--min-outflow-ratio" in result.stdout
