...
SCRIPT = ROOT / "scripts" / "run_fan_out_rule.py"


def test_fan_out_rule_cli_exists() -> None:
    assert SCRIPT.is_file()


def test_fan_out_rule_cli_help_exits_zero() -> None:
    result = run(
...
def test_cli_run_help_includes_expected_options() -> None:
    ...
    assert "--min-unique-recipients" in result.stdout
    assert "--window-days" in result.stdout
    assert "--exclude-counterparties" in result.stdout
    assert "--exclude-internal-accounts" in result.stdout
