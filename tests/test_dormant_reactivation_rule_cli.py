"""Tests for dormant reactivation rule CLI."""
...
SCRIPT = ROOT / "scripts" / "run_dormant_reactivation_rule.py"


def test_dormant_reactivation_rule_cli_exists() -> None:
    assert SCRIPT.is_file()


def test_dormant_reactivation_rule_cli_help_exits_zero() -> None:
    result = run(
...
def test_cli_run_help_includes_expected_options() -> None:
    ...
    assert "--dormant-days-threshold" in result.stdout
    assert "--min-total-outbound-amount" in result.stdout
    assert "--exclude-counterparty-outflows" in result.stdout
    assert "--exclude-internal-account-outflows" in result.stdout
